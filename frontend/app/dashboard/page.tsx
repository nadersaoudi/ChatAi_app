...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      fetch("http://localhost:8000/api/store_user", {
        method: "POST",
      ...
      fetch(`http://localhost:8000/api/get_conversations?user_id=${user.id}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleSend = async (): Promise<void> => {
    ...
    try {
      ...
      if (useRag && user) {
        ...
        const ragRes = await fetch("http://localhost:8000/api/query_rag", {
          method: "POST",
          body: formData,
        });
